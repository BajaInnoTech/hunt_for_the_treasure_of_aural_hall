...
To successfully disable a trap, you must throw a 1d6 and roll greater than *(dungeon_level+1)*. If you succeed you deactivated the trap otherwise the trap is sprung.

An attempt at trap deactivation will consume a set of thieves tools regardless of outcome. The character must have the tools in its possession. Rogues get a *+1* to trap deactivation.
...
* *Detecting Secrets also known as searching:* Players detect secrets based on a die roll that must be greater than *(dungeon_level+1)*. Elves get a re-roll when detecting secrets, while Rogues have a *+1* to their role.
...
To pick a lock a player must roll a d6 roll greater than *(dungeon_level+1)* with 1 always being failure and 6 always being a success. Rogues gain a *+1*, and Brownies also gain a *+1* to lock-picking throws - these two bonuses are cumulative.
...
|       **Roll**       | **Treasure**   |                                                 **Description**                                                        |
| -------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------- |
| <center> 1 </center> |  *Trap*        | [You found a trap](#traps)                                                                                             |
| <center> 2 </center> |  *Mimic*       | Surprise attack. If in ground level (*dungeon_level* is 0), this is a [regular treasure](#treasure).                   |
| <center> 3 </center> |  *Locked*      | Must expend a thieves toolkit to open, roll a dex save if failed the mechanism is jammed - This is a regular treasure. |
| <center> 4 </center> |  *Locked*      | Must expend a thieves toolkit to open, roll a dex save if failed the mechanism is jammed - This is a regular treasure. |
