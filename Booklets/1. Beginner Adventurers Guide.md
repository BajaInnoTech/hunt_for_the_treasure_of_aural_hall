...
This short book is an instruction manual for playing **The Hunt For The Treasure Of Aural Hall**, which is a **DepthRangers** RPG System campaign. **DepthRangers** is an Open-Source rules-light combat heavy system that uses *d6* exclusively and focuses on fast paced action focused gameplay.
...
* Sheets of paper to map your dungeon (we recommend you use graph paper for this), [you could consider using an app or website for this](https://virtual-graph-paper.com/).
...
* *Human*: Humans have no penalties or bonuses to their saves, what they have is a re-roll dice that they can apply in any context they wish (including the room or quest). They gain an additional dice every two levels, and recover one spent dice each time they rest (just one folks).
...
* *Fighter*: A trained combatant capable of using any weapon/armor they find, and produce extra attacks.
...
* *Rogue*: Capable of deadly combat maneuvers and able to handle traps, treasure and locks.
...
* A weapon for each, preferably mix ranged and melee weapons among your characters. But keep in mind someone will need a free hand to hold a torch (more on this later).
* Preferably, equip some basic armor.
* Purchase bandages & thieves tools. You start out with 5 rations & 5 torches which is indicative of how important these are (make sure to keep stock); all characters need rations to rest, and if you have a brownie in your party they're also helpful in befriending animals. Remember no thief's tools NO TREASURE and traps can't be disarmed without them either, and each character must have their own kits on hand!
...
Note that each time a party exits the dungeon and successfully completes a quest, they must descend *one level deeper* than the previous delve for their next quest.

<div class="page"/>

#### Party and Dungeon levels

As mentioned previously, only the first quest can be solved at **Level 0**, all other quests require you to descend deeper. To reach a lower level, you must explore different rooms until you find a flight of stairs (see stairs).

If all pathways are closed off and you're unable to descend as needed, then you must return to the surface and try again. As each time you enter the dungeon, the layout changes allowing another chance at completing your mission.
...
Roll a *d6* to determine what that room looks like and chart that location in a sheet of paper (we recommend you use graph paper for this), [you could consider using an app or website for this](https://virtual-graph-paper.com/).
...
Should any of your companions die, you can always "recruit" new members into your party at the guild. The settlement does not have people that can revive on retainer, sometimes they can bring people from the capital but they can arrive too late (not to mention the expense). In other words, be careful!
